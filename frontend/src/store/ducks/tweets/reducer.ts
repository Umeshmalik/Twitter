...
import { USER_ID } from "../../../constants/common-constants";
        ...
        case TweetsActionType.SET_UPDATED_TWEET:
            if (action.payload.notificationType === NotificationType.LIKE) {
                ...
                if (likedTweetIndex !== -1) {
                    draft.items[likedTweetIndex].isTweetLiked = user.id === userId ? payload.tweet.notificationCondition : draft.items[likedTweetIndex].isTweetLiked;
