    ...
    useEffect(() => {
        ...
        stompClient.connect({}, () => {
            stompClient?.subscribe(TOPIC_FEED, (response) => {
                ...
                dispatch(setUpdatedTweet(responseBody));
                dispatch(updateNotificationInfoTweet(responseBody));
            });

            stompClient?.subscribe(TOPIC_FEED_ADD, (response) => {
                const responseBody = JSON.parse(response.body);
                dispatch(setTweet(responseBody));
            });

            stompClient?.subscribe(TOPIC_FEED_SCHEDULE, (response) => {
                const responseBody = JSON.parse(response.body);
                dispatch(setScheduledTweets(responseBody));
            });

            stompClient?.subscribe(TOPIC_FEED_VOTE, (response) => {
                const responseBody = JSON.parse(response.body);
                dispatch(setVote(responseBody));
            });
        });
        ...
        if (myProfileId) {
            ...
            stompClient.connect({}, () => {
                stompClient?.subscribe(TOPIC_CHAT(myProfileId), (response) => {
                    const responseBody = JSON.parse(response.body);
                    dispatch(setChatMessage(responseBody));

                    if (myProfileId !== get(responseBody , 'author.id', -1)) {
                        dispatch(setUnreadMessage(responseBody));
                    }
                });
